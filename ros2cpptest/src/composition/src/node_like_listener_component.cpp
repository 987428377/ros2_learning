 ...
 */
//创建一个不属于通用rclcpp:：Node基类的监听器“组件”。
/**
*请注意，“组件”不必从rclcpp:：Node派生。
*如果一个对象没有从rclcpp:：Node继承，那么它必须实现：
*-接受`const rclcpp:：NodeOptions的构造函数&`
*-get_node_base_interface（），返回NodeBaseInterface：：SharedPtr
*
*这是一个实现组件所需接口的对象示例。
*/
